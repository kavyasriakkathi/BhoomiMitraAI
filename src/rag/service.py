...
class RAGService:
    ...
    def extract_pages_from_file(self, file_bytes: bytes, filename: str) -> List[Tuple[int, str]]:
        ...
        if is_pdf and PYPDF2_AVAILABLE:
            try:
                ...
                logger.info(f"PyPDF2 extracted {len(pages)} pages from {filename}")
    ...
    async def rebuild_index(self) -> RebuildIndexResponse:
        ...
        for doc in docs:
            ...
            if not pages:
                fallback_text = f"Document Title: {doc.title}. Source: {doc.source}. Category: {doc.category}. State: {doc.state}. Crop: {doc.crop}."
                pages = [(1, fallback_text)]
